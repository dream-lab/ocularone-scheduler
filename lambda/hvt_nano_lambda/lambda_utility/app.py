...
from tracking.yolov8.detect import DetectYolov8n
...
class Inference:
    def __init__(self) -> None:
        ...
        self.yolov8nano_model = LoadYolov8nModel().load_model()
        self.detect_obj = DetectYolov8n(self.yolov8nano_model)
        
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.logger = create_logger('HVT','HVT')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        outs = inference_obj.detect_obj.detect(frames)
        # outs_list = outs.detach().numpy().tolist()
        
        # outs_list = outs.detach()

        inf_time = time.time() - start

        inference_obj.logger.info("inference output for hvt = %s" % (outs))
