...
from tracking.yolov8m.detect import DetectYolov8m
import cv2
import base64
...
class Inference:
    def __init__(self) -> None:
        ...
        self.detect_yolov8m = DetectYolov8m(self.yolov8m_model)
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.logger = create_logger('CD', 'CD')
        # self.logger.info(f"CD loaded in {time.time() - start}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        outs = inference_obj.detect_yolov8m.detect(frames)
        #inference_obj.logger.info(outs)
        # print(type(outs))
        inf_time = time.time() - start

        inference_obj.logger.info(f"Inference output of crowd density  = {outs}")
        inference_obj.logger.info("task_id %s model %s  inf time: %s" % (task_id,model, inf_time))
        # inference_obj.logger.info("task_id %s model %s time taken: %s" % (task_id,model ,end_i-start_s))
