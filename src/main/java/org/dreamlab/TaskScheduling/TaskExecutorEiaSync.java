...
public class TaskExecutorEiaSync  implements Runnable, IExecutor  {
    ...
    public TaskExecutorEiaSync(String host, int port, PriorityBlockingQueue<Task> cloudQueue, LinkedBlockingQueue<Task> postProcessingQueue, int algorithm, ManagedChannel channel) {
        this.cloudQueue = cloudQueue;
        this.postProcessingQueue = postProcessingQueue;
        this.algorithm = algorithm;
        this.channel = channel;
        this.stub = InferencerGrpc.newBlockingStub(channel);
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                if ( algorithm == 2 || algorithm == 4 || algorithm == 6 || algorithm == 8 || algorithm == 10 || algorithm == 22 || algorithm == 31 || algorithm == 91 || (algorithm >= 51 && algorithm <= 56 ) || (algorithm >= 61 && algorithm <= 66 ) || (algorithm >= 71 && algorithm <= 76) || (algorithm >= 81 && algorithm <= 86)) {
    ...
    private void executeInferencer(Task task){
        ...
        try {
            ...
            logger.info(System.currentTimeMillis() + " Task received from EIA server " + task_id);
            try {
